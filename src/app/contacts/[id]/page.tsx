...
export default async function ContactDetailPage({ params }: { params: { id: string } }) {
  const contact = await prisma.contact.findUnique({
    where: { id: params.id },
    include: { tags: true, notes: { orderBy: { createdAt: 'desc' } }, conversations: { orderBy: { lastMessageAt: 'desc' }, take: 1 }, appointments: { orderBy: { scheduledAt: 'asc' } } },
  })
  if (!contact) notFound()
  const conversation = contact.conversations[0]
  const now = new Date()
  const upcomingAppointments = contact.appointments.filter(appointment => appointment.scheduledAt >= now && appointment.status !== 'CANCELLED' && appointment.status !== 'COMPLETED')
  const appointmentHistory = contact.appointments.filter(appointment => appointment.scheduledAt < now || appointment.status === 'CANCELLED' || appointment.status === 'COMPLETED').reverse()

  return (
    <MainLayout>
      <div className="mx-auto max-w-4xl space-y-6">
        <div className="flex items-center justify-between"><div><h1 className="text-2xl font-bold">Detalle del contacto</h1><p className="text-muted-foreground">Información comercial e historial.</p></div><div className="flex gap-2"><Button asChild><Link href={`/calendar/new?contactId=${contact.id}`}><CalendarPlus className="mr-2 h-4 w-4" />Programar servicio</Link></Button><Button asChild variant="outline"><Link href="/contacts"><ArrowLeft className="mr-2 h-4 w-4" />Contactos</Link></Button></div></div>
        ...
        <AppointmentList appointments={upcomingAppointments} empty="No hay próximos servicios programados." title="Servicios programados" />
        <AppointmentList appointments={appointmentHistory} empty="No hay servicios anteriores." title="Historial de servicios" />
...
function AppointmentList({ appointments, title, empty }: { appointments: Array<{ id: string; scheduledAt: Date; serviceType: keyof typeof APPOINTMENT_SERVICE_LABELS; status: keyof typeof APPOINTMENT_STATUS_LABELS }>; title: string; empty: string }) {
  return <section className="rounded-2xl border border-border bg-card p-6"><h2 className="text-lg font-semibold">{title}</h2><div className="mt-4 space-y-2">{appointments.length ? appointments.map(appointment => <Link className="flex items-center justify-between rounded-lg border border-border p-3 hover:border-primary/50" href={`/calendar/${appointment.id}`} key={appointment.id}><div><p className="font-medium">{APPOINTMENT_SERVICE_LABELS[appointment.serviceType]}</p><p className="text-sm text-muted-foreground">{appointment.scheduledAt.toLocaleString('es-ES')}</p></div><Badge variant="outline">{APPOINTMENT_STATUS_LABELS[appointment.status]}</Badge></Link>) : <p className="text-sm text-muted-foreground">{empty}</p>}</div></section>
}
